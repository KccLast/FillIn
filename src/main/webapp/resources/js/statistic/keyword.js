function resetButtons() {
    $('#wordcloud-tab').prop("disabled", false);
...
let emotionDataToSend = {}; // 각 행의 감정 분석 결과를 저장할 객체
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (currentData.length > 0) {
        ...
        analyzeEmotionByRow(currentData);
    } else {
    ...
    clusterDropdown.change(function () {
        ...
        // 클러스터별 상위 키워드 표시
        displayTopWordsByCluster(selectedCluster);
        displayTopWordsByCluster(currentData); // 클러스터별 상위 키워드 표시
    });
   ...
   /* $('#table-search-btn').click(() => {
    ...
    $('#table-search-btn').click(() => {
        let keyword = $('#keyword-input').val().trim();
        let selectedCluster = $('#phrase').val();

        // 선택된 클러스터에 따라 데이터 필터링
        if (selectedCluster !== "phrase") {
            currentData = clusteringData.filter(item => item.cluster == selectedCluster);
        } else {
            currentData = clusteringData;
        }

        // 검색어가 있을 경우, 추가 필터링
        if (keyword) {
            currentData = currentData.filter(item => item.answerContent.includes(keyword));
        }

        renderTable(currentData); // 필터링된 데이터로 테이블 렌더링
        generateWordCloudFromCurrentData(); // 필터링된 데이터로 워드클라우드 생성
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response && response.document && response.document.confidence) {
                ...
                // 감정 분석 결과를 `emotionDataToSend`에 저장
                emotionDataToSend[rowIndex] = { positive, neutral, negative };
                const rowIndex = $(this).data("index") - 1; // -1 적용

                console.log("Emotion results updated for index", rowIndex, ":", emotionDataToSend[rowIndex]);

                // 모달에 감정 결과 표시
                $("#modalContent").html(`
                    <div><strong>긍정:</strong> ${positive.toFixed(2)}</div>
                    <div><strong>중립:</strong> ${neutral.toFixed(2)}</div>
                    <div><strong>부정:</strong> ${negative.toFixed(2)}</div>
                `);
...
// $('#emotionModal').on('hidden.bs.modal', function () {
//     console.log("모달 닫힘: 데이터 준비 중...", emotionDataToSend);
//
// });
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response && response.document && response.document.confidence) {
                    ...
                    const emotionHTML = getStyledEmotionHTML(sentiment);
                    $(`.emotion-result[data-index="${index}"]`).html(emotionHTML);
...
// function getTotalEmotionByCluster(cluster){
//     let totalPositive = 0, totalNeutral = 0, totalNegative = 0;
//     let processedCount = 0;
...
//
//                 processedCount++;
//                 if (processedCount === cluster.length) {
//                     // 퍼센티지 계산을 위한 비율 변환
//                     let total = totalPositive + totalNeutral + totalNegative;
//                     let avgPositive = (totalPositive / total) * 100;
//                     let avgNeutral = (totalNeutral / total) * 100;
//                     let avgNegative = (totalNegative / total) * 100;
//                     return {total: total, positive:avgPositive,neutral:avgNeutral,negative:avgNegative};
//                 }
//             },
//             error: function (xhr, status, error) {
//                 console.error("감정 분석 API 요청 중 오류 발생:", status, error);
...
//     });
// }
async function processClusterList(clusterList) {
    const results = await Promise.all(clusterList.map(async (item) => {

        let result = await getTotalEmotionByCluster(item.itemList);
...
function getTotalEmotionByCluster(cluster) {
        ...
        return new Promise((resolve, reject) => {
            $.ajax({
                ...
                success: function (response) {
                    if (response && response.document && response.document.confidence) {
                        const {positive, neutral, negative} = response.document.confidence;
                        resolve({positive, neutral, negative});
                    } else {
                        resolve({positive: 0, neutral: 0, negative: 0});
                    }
                },
                error: function (xhr, status, error) {
                    reject(error);
    ...
    return Promise.all(promises).then(results => {
        ...
        let total = totalPositive + totalNeutral + totalNegative;
        let avgPositive = total ? (totalPositive / total) * 100 : 0;
        let avgNeutral = total ? (totalNeutral / total) * 100 : 0;
        let avgNegative = total ? (totalNegative / total) * 100 : 0;
...
// 가장 높은 감정을 계산하는 함수
function getHighestEmotion(confidence) {
    const { positive, neutral, negative } = confidence;
    if (positive >= neutral && positive >= negative) return "긍정";
    if (negative >= positive && negative >= neutral) return "부정";
    return "중립";
}
...
function getStyledEmotionHTML(emotion) {
    ...
    const iconPath = `/resources/img/statistic/${sentimentClass}.png`;

    return `
        <span class="sentiment-label ${sentimentClass}">
            <img src="${iconPath}" alt="${sentimentText} 아이콘" />
            ${sentimentText}
        </span>
    `;
}
...
function drawPieChart(ctx, data) {
    return new Chart(ctx, {
        ...
        options: {
            responsive: true,
            maintainAspectRatio: false, // 비율 조정 가능
            plugins: {
                legend: {
                    position: 'right', // 범례 위치 (top, left, bottom, right)
                    labels: {
                        font: {
                            size: 20 // 범례 글자 크기
                        },
                        padding: 20, // 범례와 차트 간격
                        boxWidth: 15 // 범례 색상 상자의 크기
                    }
                },
                tooltip: {
                    bodyFont: {
...
function drawBarChart(ctx, data) {
    return new Chart(ctx, {
        ...
        options: {
            responsive: true,
            maintainAspectRatio: false, // 차트의 비율 유지하지 않음
            plugins: {
                legend: {
                    labels: {
                        font: {
            ...
            scales: {
                x: {
                    ticks: {
                        ...
                        maxRotation: 0, // 레이블의 회전 제거
                        minRotation: 0,
                        padding: 10 // 레이블과 차트 간격
                    },
                    title: {
                        display: true,
                        text: '감정 유형',
...
// 클러스터별 감정 결과 수집 함수
// function calculateClusterSentimentData(data) {
//     const sentimentDataByCluster = {};
//
//     data.forEach((item) => {
//         const cluster = item.cluster;
//         // item.details가 없으면 기본값 설정
//         const { positive = 0, neutral = 0, negative = 0 } = item.details || {};
//
//         if (!sentimentDataByCluster[cluster]) {
//             sentimentDataByCluster[cluster] = { positive: 0, neutral: 0, negative: 0, count: 0 };
//         }
//
//         // 감정 값을 클러스터에 누적
//         sentimentDataByCluster[cluster].positive += positive;
//         sentimentDataByCluster[cluster].neutral += neutral;
//         sentimentDataByCluster[cluster].negative += negative;
//         sentimentDataByCluster[cluster].count += 1;
//     });
//
//     // 클러스터별 평균 감정 계산
//     Object.keys(sentimentDataByCluster).forEach((cluster) => {
//         const sentiment = sentimentDataByCluster[cluster];
//         sentiment.positive /= sentiment.count;
//         sentiment.neutral /= sentiment.count;
//         sentiment.negative /= sentiment.count;
//     });
//
//     return sentimentDataByCluster;
// }
// 클러스터별 감정 결과 수집 함수


// 클러스터별 상위 3개의 단어 추출 함수
function getTopWordsByCluster(data, topN = 3) {
    const clusters = {};

    data.forEach((item) => {
        const cluster = item.cluster;
        const words = item.answerContent.trim().split(/\s+/);

        clusters[cluster] = clusters[cluster] || {};

        words.forEach((word) => {
            if (word.length > 1 && !stopWords.includes(word)) {
                clusters[cluster][word] = (clusters[cluster][word] || 0) + 1;
            }
        });
    });

    // 상위 3개 단어 추출
    const topWordsByCluster = {};
    Object.keys(clusters).forEach((cluster) => {
        const sortedWords = Object.entries(clusters[cluster])
            .sort(([, a], [, b]) => b - a)
            .slice(0, topN)
            .map(([word]) => word);

        topWordsByCluster[cluster] = sortedWords;
    });

    return topWordsByCluster;
}
// 클러스터별 상위 단어 표시 함수

// 클러스터 데이터 수집 함수
// function collectClusterData() {
//     const clusterSentimentData = calculateClusterSentimentData(clusteringData); // 클러스터별 감정 결과 수집
//     const clusterTopWordsData = getTopWordsByCluster(clusteringData); // 클러스터별 상위 키워드 수집
//
//     return {
...
//
//
function displayTopWordsByCluster(data) {
    const topWordsByCluster = getTopWordsByCluster(data);
    const clusterTopwordsContainer = document.getElementById("cluster-topwords");
    clusterTopwordsContainer.innerHTML = ""; // 초기화

    Object.keys(topWordsByCluster).forEach((cluster) => {
        const topWords = topWordsByCluster[cluster].join(", ");
        const clusterElement = document.createElement("div");
        clusterElement.textContent = `Cluster ${cluster}: ${topWords}`;
        clusterTopwordsContainer.appendChild(clusterElement);
    });
}

// 페이지 로드 시 클러스터별 상위 단어 표시
document.addEventListener("DOMContentLoaded", function () {
    displayTopWordsByCluster(clusteringData); // clusteringData는 페이지 전체 데이터
});
// 클러스터별 상위 3개의 단어 추출 함수
...
//     console.log("#####");
//     console.log(emotionDataToSend);
//
//     let tableData = [];
//     const topWordsByCluster = getTopWordsByCluster(clusteringData); // 전체 데이터에서 클러스터별 상위 키워드 추출
...
//             details: emotionDataToSend[index] || { positive: 0, neutral: 0, negative: 0 },
//             sentimentResult: $(this).find(".emotion-result").text().trim(),
//             clusterTopWords: topWordsByCluster[$(this).find("td:eq(1)").text().trim()] || [] // 선택한 클러스터의 상위 키워드 포함
//         };
//         // console.log(rowData);
//         // console.log(`Row ${index} details:`, rowData.details); // 로그 추가
...
async function collectTableData() {
    ...
    for (let i = 0; i < rows.length; i++) {
        const row = $(rows[i]);
        const text = row.find(".ans-content").text().trim();
        const details = await getDetailEmotion(text); // 감정 분석 결과를 기다림

        let rowData = {
            seq: row.find("td:eq(0)").text().trim(),
            cluster: row.find("td:eq(1)").text().trim(),
            answerContent: text,
            answerDate: row.find("td:eq(3)").text().trim(),
            frequency: row.find("td:eq(4)").text().trim(),
            topwords: row.find("td:eq(5)").text().trim(),
            details: details,
            sentimentResult: row.find(".emotion-result").text().trim(),
            clusterTopWords: topWordsByCluster[row.find("td:eq(1)").text().trim()] || []
        };
        tableData.push(rowData);
    }
    console.log(tableData);
...
function getDetailEmotion(text) {
    return new Promise((resolve, reject) => {
        $.ajax({
            url: "/api/statistic/analyzeEmotion",
            method: "POST",
            ...
            success: function (response) {
                if (response && response.document && response.document.confidence) {
                    let { positive, neutral, negative } = response.document.confidence;
                    positive = `${positive.toFixed(2)}`;
                    neutral = `${neutral.toFixed(2)}`;
                    negative = `${negative.toFixed(2)}`;
                    resolve({ positive, neutral, negative });
                } else {
                    resolve({ positive: 0, neutral: 0, negative: 0 }); // Default or error case
                }
            },
            error: function () {
                reject(new Error("Sentiment analysis request failed."));
            }
        });
    });
}
...
function collectClusterData() {
    // const clusterSentimentData = calculateClusterSentimentData(clusteringData); // 클러스터별 감정 결과 계산
    const clusterTopWordsData = getTopWordsByCluster(clusteringData); // 클러스터별 상위 키워드 계산
...
$("#nextBtn").click(async function () {
    ...
    let clusterLength = $('#phrase > option').length -1;
     ...
     // 걍 index로 옮
    //let tableData = await collectTableData();
    ...
    let clusterList = clusteringData.reduce((acc, item) => {
        // 해당 클러스터의 인덱스 찾기
        let index = acc.findIndex(cluster => cluster.clusterId === item.cluster);

        if (index === -1) {
            // 클러스터가 존재하지 않으면 새로 추가
            acc.push({
            ...
            // 존재하는 클러스터에 데이터 추가
            acc[index].itemList.push(item);
        }

        return acc;
    }, []);

    // clusterList.forEach(function(item,idx){
    //     console.log(item.itemList);
    //     let result = getTotalEmotionByCluster(item.itemList);
    //     console.log(result);
    // })
    let avgResults;
    try {
        avgResults = await processClusterList(clusterList);
        console.log(avgResults);
    } catch (error) {
        console.error("Error processing cluster list:", error);
    }
    avgResults.forEach(function(item,idx){
        let findCluster = clusterList.filter((a) => a.clusterId === item.clusterId);
        console.log(findCluster);
        findCluster[0].emotion = item.emotions;
    })

    clusterList.forEach((item)=>{
        let topword = topWordsByCluster[(item.clusterId)];
        item.topword = topword
    });
    sendData.clusterList = clusterList;

    console.log(sendData);
    // const clusterData = collectClusterData(); // 클러스터 데이터 수집
    //
    // const dataToSend = {
    //     tableData,
    //     clusterData
    ...
    // 각 행에 대해 감정 분석 결과를 sentimentResults로 추가
    // tableData.forEach((row, index) => {
    //     if (emotionDataToSend[index]) {
    //         row.details = emotionDataToSend[index]; // 각 행의 고유한 감정 분석 결과 추가
    //     }
    // });

    const tableDataJsonString = JSON.stringify(sendData);
