...
$(document).ready(function () {
  ...
  function loadData() {
    ...
    $('#search-input').on('click', function () {
      ...
      var questionId =
        $('#questionSelect').val() === '' ? 0 : $('#questionSelect').val();
      var itemContent =
        $('#itemSelect').val() === null ? '' : $('#itemSelect').val();
...
function updateParticipantsPage(response) {
  ...
  document.getElementById(
    'progress-bar-fill'
  ).textContent = `${response.participantsCount}명`;
  document.getElementById(
    'progress-bar-total'
  ).textContent = `${response.targetCount}명`;
}
...
<!--            </div>-->
    ...
    renderChart(
      quanQuestionList.find((i) => i.questionOrder === item.questionOrder)
        .questionItems
    ); // 차트 렌더링 함수 호출
  } else if (item.parentSeq === 4) {
    // 테이블 삽입
    cardBody.innerHTML = `
            <h5 class="fw-bold">${item.questionOrder}번: ${
      item.questionName
    }</h5>
            <p>${item.questionDescription}</p>
            <div style="padding: 0; max-height: 300px; overflow-y: auto;">
                <table class="table" style="table-layout: fixed; width: 100%;">
                    ...
                    <tbody>
                        ${qualQuestionList
                          .find((i) => i.questionOrder === item.questionOrder)
                          .answerList.map(
                            (answer) => `
                            <tr>
...
function updateSelectQuestion(response) {
  ...
  response.quantitativeResponseList.forEach((question) => {
    questionSelect.append(
      new Option(question.questionName, question.questionOrder)
    );
  });
  ...
  questionSelect.on('change', function () {
    ...
    const selectedQuestion = response.quantitativeResponseList.find(
      (q) => q.questionOrder == selectedQuestionOrder
    );
...
function renderChart(questionItems) {
  ...
  const data = questionItems.map((item) =>
    item.itemRatio !== null ? item.itemRatio : 0
  );
  ...
  labels.forEach((label, i) => {
    ...
    colorBox.style.backgroundColor =
      quanChart.data.datasets[0].backgroundColor[i];
